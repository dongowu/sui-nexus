...
func Load() *Config {
	replayWindow, err := strconv.ParseInt(getEnv("REPLAY_WINDOW_SEC", "300"), 10, 64)
	if err != nil || replayWindow <= 0 {
		replayWindow = 300
	}
	gasBudget := parseUintEnv("SUI_GAS_BUDGET", 10_000_000)
	maxEpoch, err := strconv.ParseInt(getEnv("ZKLOGIN_MAX_EPOCH", "30"), 10, 64)
	if err != nil || maxEpoch <= 0 {
		maxEpoch = 30
	}
